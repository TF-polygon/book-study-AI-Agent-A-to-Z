...
### 2.1. 첫 번째 분류 기준: 가상 환경의 에이전트와 체화된 에이전트의 구분
...
|교차 분해 방식|작업 분해와 계획 수립을 교차하면서 진행|- 추론과 계획을 번갈아 수행하므로 모델의 계획 능력을 점진적으로 향상시킴<br>- 환경 변화에 따라 작업을 동적으로 조정|문제가 지나치게 복잡할 시, 결과를 내지 못한 채 생셩 결과만 길어지는 추론-계획 연쇄에 빠질 위험이 있다.|
